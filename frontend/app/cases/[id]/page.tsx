    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div className="flex items-center space-x-3">
          <Link href="/cases" className="p-2 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-400 hover:text-white transition">
        ...
        <div className="flex items-center space-x-3">
          {c?.state !== "RECOVERED" && (
            <button
              ...
              className="flex items-center space-x-1.5 px-4 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 via-blue-600 to-indigo-600 hover:from-cyan-400 hover:to-indigo-500 text-white font-bold text-xs shadow-lg shadow-blue-500/20 transition disabled:opacity-50"
            >
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-lg shadow-emerald-600/20 transition disabled:opacity-50"
          >
      ...
      {statusMsg && (
        <div className="bg-slate-900 border border-slate-700 rounded-2xl p-4 flex items-center justify-between text-xs text-cyan-200">
          ...
          <button onClick={() => setStatusMsg(null)} className="text-xs text-cyan-400 hover:underline">Dismiss</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
          <span className="text-[11px] text-slate-400 uppercase font-semibold block">Transaction Amount</span>
          <span className="text-2xl font-black text-white mt-1 block">₹{c?.amount_inr?.toLocaleString()}</span>
          <span className="text-[11px] text-slate-500 mt-1 block font-mono">{c?.currency}</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
          <span className="text-[11px] text-slate-400 uppercase font-semibold block">Intent Score (0–100)</span>
          <span className="text-2xl font-black text-emerald-400 mt-1 block">{c?.intent_score}/100</span>
          <span className="text-[11px] text-slate-500 mt-1 block">Recovery Opp: {c?.recovery_opportunity_score}/100</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
          <span className="text-[11px] text-slate-400 uppercase font-semibold block">Payment Method & Bank</span>
          <span className="text-sm font-bold text-white mt-1 block uppercase">{c?.issuer} {c?.payment_method}</span>
          <span className="text-[11px] text-slate-500 mt-1 block font-mono">{c?.latest_failure_code}</span>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
          <span className="text-[11px] text-slate-400 uppercase font-semibold block">Rail Telemetry Status</span>
          <span className={`text-sm font-bold mt-1 block ${rail?.is_degraded ? "text-rose-400" : "text-emerald-400"}`}>
            {rail?.is_degraded ? "DEGRADED RAIL" : "HEALTHY RAIL"}
          </span>
          <span className="text-[11px] text-slate-500 mt-1 block">Success Rate: {Math.round((rail?.rolling_success_rate || 0) * 100)}%</span>
        </div>
      </div>

      {/* LangGraph Multi-Agent Continuous Review Loop Inspector */}
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div>
            <h2 className="text-base font-bold text-white flex items-center space-x-2">
              ...
              <span>LangGraph Multi-Agent Continuous Evaluation Loop</span>
            </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-bold text-white">1. Diagnostician</span>
          ...
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-bold text-white">2. Strategist</span>
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
            </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-bold text-white">3. Policy Critic</span>
            ...
            <p className="text-[11px] text-slate-400">Hard policy checks passed</p>
          </div>

          {/* Node 4: Supervisor */}
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-bold text-white">4. Supervisor</span>
              <CheckCircle2 className="w-3.5 h-3.5 text-cyan-400" />
            </div>
            <div className="text-xs font-bold text-white">Consensus Gated</div>
            <p className="text-[11px] text-slate-400">Approved for execution</p>
          </div>

          {/* Node 5: Communicator */}
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-bold text-white">5. Communicator</span>
              <MessageSquare className="w-3.5 h-3.5 text-blue-400" />
            </div>
            <div className="text-xs font-bold text-blue-400">Eng & Hinglish</div>
            <p className="text-[11px] text-slate-400">Personalized copy ready</p>
          </div>
        </div>

        {/* Customer Copy Preview */}
        <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
          <span className="text-xs font-bold text-slate-300 uppercase tracking-wider block">Generated Customer Communications</span>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
            <div className="p-3 rounded-xl bg-slate-900 border border-slate-800 text-slate-300">
              <span className="text-[10px] font-bold text-slate-500 uppercase block mb-1">English Message Copy</span>
            ...
            <div className="p-3 rounded-xl bg-slate-900 border border-slate-800 text-slate-300">
              <span className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Hinglish Message Copy</span>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-white flex items-center space-x-2">
              ...
              <span>Razorpay Test Mode Execution Record</span>
            </h3>
            <span className="text-xs font-mono text-slate-400">Idempotency: {latestIntervention.idempotency_key}</span>
          </div>

          {latestIntervention.payment_link_url && (
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 flex items-center justify-between">
              <div>
                <span className="text-[11px] text-slate-500 uppercase block">Generated Standard Payment Link</span>
